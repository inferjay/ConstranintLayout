...
public class ConstraintTableLayout extends ConstraintWidgetContainer {
    ...
    class HorizontalSlice {
        android.constraint.solver.widgets.ConstraintWidget top;
        android.constraint.solver.widgets.ConstraintWidget bottom;
        int padding;
    }
     ...
     */
    class VerticalSlice {
        android.constraint.solver.widgets.ConstraintWidget left;
        android.constraint.solver.widgets.ConstraintWidget right;
    ...
    private ArrayList<android.constraint.solver.widgets.Guideline> mVerticalGuidelines = new ArrayList<>();
    private ArrayList<android.constraint.solver.widgets.Guideline> mHorizontalGuidelines = new ArrayList<>();
     ...
     */
    public ArrayList<android.constraint.solver.widgets.Guideline> getVerticalGuidelines() {
     ...
     * @return array of guidelines
     */
    public ArrayList<android.constraint.solver.widgets.Guideline> getHorizontalGuidelines() {
    ...
    public void addToSolver(LinearSystem system) {
        ...
        if (system == mSystem) {
            ...
            for (int i = 0; i < num; i++) {
                android.constraint.solver.widgets.Guideline guideline = mVerticalGuidelines.get(i);
            ...
            for (int i = 0; i < num; i++) {
                android.constraint.solver.widgets.Guideline guideline = mHorizontalGuidelines.get(i);
                guideline.setPositionRelaxed(
                        getVerticalDimensionBehaviour() == DimensionBehaviour.WRAP_CONTENT);
                guideline.addToSolver(system);
            }
            for (int i = 0; i < count; i++) {
                android.constraint.solver.widgets.ConstraintWidget child = mChildren.get(i);
    ...
    public void setTableDimensions() {
        ...
        int count = mChildren.size();
        for (int i = 0; i < count; i++) {
            android.constraint.solver.widgets.ConstraintWidget widget = mChildren.get(i);
    ...
    private void setVerticalSlices() {
        ...
        for (int i = 0; i < mNumCols; i++) {
            ...
            slice.left = previous;
            if (i < mNumCols - 1) {
                android.constraint.solver.widgets.Guideline guideline = new android.constraint.solver.widgets.Guideline();
                guideline.setOrientation(android.constraint.solver.widgets.Guideline.VERTICAL);
    ...
    private void setHorizontalSlices() {
        ...
        for (int i = 0; i < mNumRows; i++) {
            ...
            if (i < mNumRows - 1) {
                android.constraint.solver.widgets.Guideline guideline = new android.constraint.solver.widgets.Guideline();
                guideline.setOrientation(android.constraint.solver.widgets.Guideline.HORIZONTAL);
                guideline.setParent(this);
                guideline.setRelativePercent((int) percent);
                percent += increment;
    ...
    private void setChildrenConnections() {
        ...
        int index = 0;
        for (int i = 0; i < count; i++) {
            android.constraint.solver.widgets.ConstraintWidget target = mChildren.get(i);
            ...
            android.constraint.solver.widgets.ConstraintWidget targetLeft = verticalSlice.left;
            android.constraint.solver.widgets.ConstraintWidget targetRight = verticalSlice.right;
            android.constraint.solver.widgets.ConstraintWidget targetTop = horizontalSlice.top;
            android.constraint.solver.widgets.ConstraintWidget targetBottom = horizontalSlice.bottom;

            target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.LEFT)
                    .connect(targetLeft.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.LEFT), mPadding);
            if (targetRight instanceof android.constraint.solver.widgets.Guideline) {
                target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.RIGHT)
                        .connect(targetRight.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.LEFT), mPadding);
            } else {
                target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.RIGHT)
                        .connect(targetRight.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.RIGHT), mPadding);
            }
...
//            target.getAnchor(ConstraintAnchor.Type.RIGHT).setConnectionType(ConstraintAnchor.ConnectionType.STRICT);
            switch (verticalSlice.alignment) {
                ...
                break;
                case ALIGN_LEFT: {
                    target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.LEFT).setStrength(
                            android.constraint.solver.widgets.ConstraintAnchor.Strength.STRONG);
                    target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.RIGHT).setStrength(
                            android.constraint.solver.widgets.ConstraintAnchor.Strength.WEAK);
                }
                break;
                case ALIGN_RIGHT: {
                    target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.LEFT).setStrength(
            ...
            target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.TOP)
                    .connect(targetTop.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.TOP), mPadding);
            if (targetBottom instanceof android.constraint.solver.widgets.Guideline) {
                target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.BOTTOM)
                        .connect(targetBottom.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.TOP), mPadding);
            } else {
                target.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.BOTTOM)
                        .connect(targetBottom.getAnchor(android.constraint.solver.widgets.ConstraintAnchor.Type.BOTTOM), mPadding);
            }
