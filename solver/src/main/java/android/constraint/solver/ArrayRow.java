...
class ArrayRow implements android.constraint.solver.IRow {
    ...
    public void updateClientEquations() {
        ...
        for (int i = 0; i < count; i++) {
            android.constraint.solver.SolverVariable v = variables.getVariable(i);
    ...
    public android.constraint.solver.IRow createRowCentering(android.constraint.solver.SolverVariable variableA, android.constraint.solver.SolverVariable variableB, int marginA,
            float bias, android.constraint.solver.SolverVariable variableC, android.constraint.solver.SolverVariable variableD, int marginB,
    ...
    public boolean updateRowWithEquation(android.constraint.solver.IRow row) {
        ...
        if (amount != 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                android.constraint.solver.SolverVariable v = equation.variables.variables[idx];
    ...
    public void pickRowVariable() {
        android.constraint.solver.SolverVariable restrictedCandidate = null;
        android.constraint.solver.SolverVariable unrestrictedCandidate = null;
        // let's find an adequate variable to pivot on
        int count = variables.size();
        for (int i = 0; i < count; i++) {
            int idx = variables.indexes[i];
            android.constraint.solver.SolverVariable candidateVariable = variables.variables[idx];
    ...
    public android.constraint.solver.SolverVariable findPivotCandidate() {
        android.constraint.solver.SolverVariable candidate = variables.getPivotCandidate();
        if (candidate != null) {
            return candidate;
        }
        int count = variables.size();
